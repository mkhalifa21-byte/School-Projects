...
void init(int& shmid, int& msqid, void*& sharedMemPtr) {
	...
	*/
    ...
    outfile.close();
    printf("Created keyfile.txt successfully.\n");

    if ((key = ftok("keyfile.txt", 'a')) == -1) {
        perror("ftok");
        exit(-1);
    }
    printf("Generated key successfully. Key: %d\n", key);

    if ((shmid = shmget(key, SHARED_MEMORY_CHUNK_SIZE, S_IRUSR | S_IWUSR | IPC_CREAT)) == -1) {
        perror("shmget");
        exit(-1);
    }
    printf("Allocated shared memory. shmid: %d\n", shmid);

    if ((sharedMemPtr = shmat(shmid, NULL, 0)) == (void *)-1) {
        perror("shmat");
        exit(-1);
    }
    printf("Attached to shared memory. sharedMemPtr: %p\n", sharedMemPtr);
